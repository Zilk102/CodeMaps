...
import type { DetectedPattern } from './patternDetectionTypes';

export function buildStructuralPatterns(args: {
  graph: GraphData;
  architecture: ArchitectureOverview;
  incomingByTarget: Map<string, GraphData['links']>;
  outgoingBySource: Map<string, GraphData['links']>;
  nodeById: Map<string, GraphData['nodes'][number]>;
  childrenByParentId: Map<string, GraphData['nodes']>;
}): DetectedPattern[] {
  const patterns: DetectedPattern[] = [];
  const { graph, architecture, incomingByTarget, outgoingBySource, nodeById, childrenByParentId } =
    args;
...
export function buildQualityPatterns(quality: ModuleQualitySummary): DetectedPattern[] {
  ...
  if (quality.godClasses.length > 0) {
    patterns.push({
      ...
      evidence: quality.godClasses
        .flatMap(({ node, matchedClasses }) =>
          matchedClasses.slice(0, 2).map((item) => ({
            nodeId: node.id,
            message: describeClassEvidence(node.label, item),
          }))
        )
        .slice(0, 8),
    });
  ...
  if (quality.longMethods.length > 0) {
    patterns.push({
      ...
      evidence: quality.longMethods
        .flatMap(({ node, matchedMethods }) =>
          matchedMethods.slice(0, 2).map((item) => ({
            nodeId: node.id,
            message: describeMethodEvidence(node.label, item),
          }))
        )
        .slice(0, 8),
    });
  ...
  if (quality.complexMethods.length > 0) {
    patterns.push({
      ...
      evidence: quality.complexMethods
        .flatMap(({ node, matchedMethods }) =>
          matchedMethods.slice(0, 2).map((item) => ({
            nodeId: node.id,
            message: describeMethodEvidence(node.label, item),
...
export function buildRuntimePatterns(args: {
  graph: GraphData;
  incomingByTarget: Map<string, GraphData['links']>;
  outgoingBySource: Map<string, GraphData['links']>;
}): DetectedPattern[] {
  const patterns: DetectedPattern[] = [];
  const { graph, incomingByTarget, outgoingBySource } = args;
...
function describeClassEvidence(fileName: string, item: SourceClassMetric): string {
  const range =
...
function describeMethodEvidence(fileName: string, item: SourceFunctionMetric): string {
  const range =
    item.startLine > 0 && item.endLine >= item.startLine
      ? `L${item.startLine}-L${item.endLine}`
      : 'line range unavailable';
