...
export class QualityGovernanceService {
  buildBudget(input: BuildQualityGovernanceInput): QualityBudget {
    ...
    const overallStatus =
      ...
      blockingIssueCodes.length >= 4
        ? 'critical'
        : dimensions.some((dimension) => dimension.status === 'warning') || blockingIssueCodes.length > 0
          ? 'warning'
          : 'healthy';
  ...
  buildRefactoringWaves(
    ...
    for (const wave of waveSpecs) {
      const candidates = decompositionGuidance.candidates
          ...
          .sort(
            (left, right) =>
              this.scoreCandidateDependencyPressure(right, collapsed) -
                this.scoreCandidateDependencyPressure(left, collapsed) ||
      ...
      producedWaves.push({
        ...
        priorityScore: Math.round(
          candidates.reduce(
            (sum, candidate) => sum + candidate.score + this.scoreCandidateDependencyPressure(candidate, collapsed),
            0
          ) / candidates.length
        ),
        exitCriteria: this.createExitCriteria(wave.id, candidates),
      });
    }
  ...
  buildDashboard(
    ...
    const gates: QualityGate[] = [
        ...
        status:
          budget.recommendedPolicy === 'freeze_growth'
            ...
            : budget.recommendedPolicy === 'refactor_before_growth'
              ? 'warning'
              : 'pass',
        rationale: this.summarizeBudgetForStep(budget),
      },
      {
      ...
      },
      {
        ...
        status: refactoringWaves.length === 0 ? 'pass' : refactoringWaves[0].blockingFileIds.length > 0 ? 'warning' : 'pass',
        rationale:
  ...
  private scoreCandidateDependencyPressure(
  ...
  private createExitCriteria(waveId: string, candidates: DecompositionCandidate[]) {
    switch (waveId) {
