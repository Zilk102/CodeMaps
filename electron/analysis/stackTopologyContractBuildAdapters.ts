  ...
  Array.from(
    new Set([
      ...Array.from(
        ...
        .map((match) => match[1])
        .filter((value): value is string => Boolean(value)),
      ...Array.from(text.matchAll(/\b(?:export\s+)?const\s+([A-Za-z_][A-Za-z0-9_]*)\s*=/g))
        .map((match) => match[1])
        .filter((value): value is string => Boolean(value)),
...
const extractConnectRuntimeSymbols = (text: string) =>
  Array.from(
    new Set([
      ...Array.from(
        text.matchAll(
          /\b(?:export\s+)?const\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*createPromiseClient\s*\(/g
        )
      )
        .map((match) => match[1])
        .filter((value): value is string => Boolean(value)),
...
const extractGrpcWebRuntimeSymbols = (text: string) =>
  Array.from(
    new Set([
      ...Array.from(
        text.matchAll(
          /\b(?:export\s+)?const\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*new\s+[A-Z][A-Za-z0-9_]*Client\s*\(/g
        )
      )
        .map((match) => match[1])
        .filter((value): value is string => Boolean(value)),
...
const extractOpenApiRuntimeSymbols = (filePath: string, text: string) => {
  ...
  switch (extension) {
    ...
    case '.go':
      return Array.from(
        new Set([...extractGoRouteHandlers(text), ...extractGoFunctionNames(text)])
      );
    case '.rb':
      return Array.from(
        new Set([...extractRubyMethodNames(text), ...inferClassSymbolsForFile(text)])
      );
    case '.php':
      return Array.from(
        new Set([...extractPhpMethodNames(text), ...inferClassSymbolsForFile(text)])
      );
    case '.rs':
      return Array.from(
        new Set([
...
const isGeneratedProtoModule = (filePath: string, text: string) =>
  ...
  /Code generated by protoc|Generated by the protocol buffer compiler|protobufjs|minimal\.js/i.test(
    text
  );
...
const isGrpcWebRuntimeEntry = (text: string) =>
  /GrpcWebFetchTransport\s*\(|grpc\.unary\s*\(|grpc\.invoke\s*\(|new\s+[A-Za-z0-9_]+Client\s*\(/.test(
    text
  );
...
export const openApiAdapter: StackAdapter = {
  ...
  analyze: async (context) => {
    ...
    const candidateFiles = context
      .getAllFilePaths()
      .filter((filePath) =>
        /\.(yaml|yml|json|ts|tsx|js|jsx|java|kt|go|py|cs)$/i.test(
    ...
    const specEntries = relativeEntries.filter((entry) =>
      isLikelyOpenApiSpec(entry.filePath, entry.text)
    );
    const configFiles = relativeEntries
      .filter((entry) =>
        OPENAPI_GENERATOR_CONFIG_BASENAMES.has(path.posix.basename(entry.filePath))
      )
              ...
              ? [
                  createRelationship(
                    ...
                    resolveSymbolEndpoint(runtimeEntry.filePath, graphSymbolIndex, {
                      preferredSymbols: [
                        ...matchingSymbols,
                        ...buildIdentifierVariants(operationId),
                      ],
                    }),
                    'framework',
...
export const protobufAdapter: StackAdapter = {
  ...
  analyze: async (context) => {
    ...
    const candidateFiles = context
      .getAllFilePaths()
      .filter((filePath) =>
        /\.(proto|ts|tsx|js|jsx|go|java|kt|cs|py|yaml|yml)$/i.test(
          context.getProjectRelativePath(filePath)
        )
      );
    const contents = await context.readTexts(candidateFiles);
    ...
    const generatedEntries = relativeEntries.filter(
      (entry) =>
        !entry.filePath.endsWith('.proto') && isGeneratedProtoModule(entry.filePath, entry.text)
    );
    ...
    const connectRuntimeEntries = runtimeEntries.filter((entry) =>
      isConnectRuntimeEntry(entry.text)
    );
    const grpcWebRuntimeEntries = runtimeEntries.filter((entry) =>
      isGrpcWebRuntimeEntry(entry.text)
    );
    ...
    const relationships: StackStructuralRelationship[] = [
      ...
      ...configFiles.flatMap((configFile) =>
        bufOutputDirs.flatMap((outputDir) =>
          generatedEntries
            .filter((generatedEntry) =>
              generatedEntry.filePath.startsWith(`${outputDir.replace(/\/+$/, '')}/`)
            )
            .map((generatedEntry) =>
